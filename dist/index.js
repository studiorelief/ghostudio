...
(() => {
  ...
  // src/utils/animations/element/animatedCodeBock.ts
  var CODE_BLOCKS = {
    "raise-funds": `<div id="radar-wrap" style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:transparent;">
...
<\/script>`,
    "launch-strong": `<div id="launch-wrap" style="width:100%;max-width:350px;margin:0 auto;aspect-ratio:350/265;">
...
<\/script>`,
    "scale-fast": `<div id="retention-wrap" style="width:100%;max-width:350px;margin:0 auto;aspect-ratio:350/265;">
...
<\/script>`,
    performance: `<div id="analytics-wrap" style="width:100%;max-width:350px;margin:0 auto;display:flex;align-items:center;justify-content:center;aspect-ratio:350/265;">
...
<\/script>`,
    network: `<div id="network-wrap" style="width:100%;max-width:350px;margin:0 auto;aspect-ratio:350/265;">
...
<\/script>`
  ...
  function injectBlock(container, html) {
  ...
  gsapWithCSS.registerPlugin(ScrollTrigger2, CustomEase, SplitText);

  // src/utils/animations/element/levitate.ts
  function initLevitate(amplitude = 1, duration = 2) {
  ...
  // src/utils/animations/element/preloader.ts
  function initPreloaderAnimation() {
    ...
    const ghostWrapper = document.querySelector(".preloader_ghost-wrapper");
  ...
  function showPreloader() {
    const preloaderComponent = document.querySelector(".preloader_component");
    if (preloaderComponent) {
      preloaderComponent.style.display = "flex";
    }
  }
  function animateFloatingGhost() {
    const tl = gsapWithCSS.timeline({
      repeat: -1,
      yoyo: true,
      ease: "power1.inOut"
  ...
  function animateGhostFades() {
    const ghostFadeTl = gsapWithCSS.timeline({
      repeat: -1
    });
    for (let i = 1; i <= 9; i++) {
      const ghostSelector = `.preloader_ghost.is-${i}`;
      gsapWithCSS.set(ghostSelector, { opacity: 0 });
      ghostFadeTl.to(ghostSelector, {
        opacity: 1,
        duration: 0.5,
        ease: "power2.inOut"
      }).to(ghostSelector, {
  ...
  function animateCounterAndProgress(stopMouseTracking) {
    ...
    const endLoaderAnimation = () => {
      const preloaderComponent = document.querySelector(".preloader_component");
      gsapWithCSS.to(preloaderNumber, {
        y: 4 * 16,
        opacity: 0,
        duration: 0.4,
        ease: "power2.out",
        onComplete: () => {
          gsapWithCSS.to(preloaderComponent, {
            opacity: 0,
            duration: 0.5,
            // Longer fade out to ensure smooth transition
            ease: "power2.out",
            onComplete: () => {
              if (preloaderComponent) {
                preloaderComponent.style.display = "none";
  ...
  function setupMouseTracking(ghostWrapper) {
    const onMouseMove = (event2) => {
      const mouseX = event2.clientX / window.innerWidth - 0.5;
      const mouseY = event2.clientY / window.innerHeight - 0.5;
      gsapWithCSS.to(ghostWrapper, {
        x: `${-mouseX * 50}rem`,
        y: `${-mouseY * 50}rem`,
        duration: 1,
        ease: "power2.out",
  ...
  // src/utils/animations/element/scaleUp.ts
  ...
  // src/utils/animations/element/slideUp.ts
  ...
  // src/utils/animations/gsap/cardAppear.ts
  function initCardAppear() {
    ...
    rowGroups.forEach((rowCells) => {
      gsapWithCSS.fromTo(
        ...
        },
        {
          ...
          scrollTrigger: {
            trigger: rowCells[0],
            // Premier élément de la ligne comme trigger
            start: "0% 100%",
            end: "100% 75%",
            markers: false,
  ...
  // src/utils/animations/gsap/footerGhost.ts
  function initFooterGhostAnimation() {
    animateWrapperLevitation();
    animateGhostCrossfade();
  }
  function animateWrapperLevitation() {
  ...
  function animateGhostCrossfade() {
    const numberOfGhosts = 5;
    const interval = 1.5;
    if (!document.querySelector(".footer_ghost.is-1")) return;
    for (let i = 1; i <= numberOfGhosts; i++) {
      gsapWithCSS.set(`.footer_ghost.is-${i}`, { opacity: 0 });
    }
    gsapWithCSS.set(".footer_ghost.is-1", { opacity: 1 });
  ...
  // src/utils/animations/gsap/ghostAnimation.ts
  function initGhostAnimation() {
    ...
    ghostElements.forEach((element) => {
      ...
      if (wrapper) {
        gsapWithCSS.timeline({ repeat: -1, yoyo: true }).fromTo(
          wrapper,
          { y: "1rem", rotation: -5 },
  ...
  // src/utils/animations/gsap/haloRotate.ts
  function initHaloRotate(selector3 = "[rotate]", duration = 10, ease = "linear") {
  ...
  // src/utils/animations/gsap/logoAppear.ts
  function initLogoAppear() {
  ...
  // src/utils/animations/gsap/logoScroll.ts
  function logoScroll() {
    const logo = document.querySelector(".home_headline_logo");
    const section = document.querySelector(".section_home_headline");
    if (!logo || !section) return;
    gsapWithCSS.set(logo, { rotation: 90 });
    gsapWithCSS.to(logo, {
      rotation: 0,
      ease: "none",
      scrollTrigger: {
  ...
  // src/utils/animations/gsap/loopOpen.ts
  function initLoopOpen() {
    const squares = document.querySelectorAll(".loop_square.is-fix");
    if (!squares.length) return;
    squares.forEach((square) => {
      gsapWithCSS.timeline({ repeat: -1, yoyo: true }).fromTo(square, { scale: 1 }, { scale: 0.8, duration: 0.6, ease: "power2.Out" });
    });
  }

  // src/utils/animations/gsap/mainBackground.ts
  var sections = [
  ...
  function initMainBackground(targetSelector = ".section_background") {
    const target = document.querySelector(targetSelector);
    if (!target) return;
    gsapWithCSS.to(".section_project", { opacity: 1, duration: 0.5, ease: "power2.out" });
  ...
  // src/utils/animations/gsap/revealText.ts
  function initRevealHeader() {
    const elements = document.querySelectorAll('[reveal-header="blur"]');
    if (!elements.length) return;
    elements.forEach((el) => {
  ...
  function initRevealButton() {
    const elements = document.querySelectorAll('[reveal="button"]');
    if (!elements.length) return;
    elements.forEach((el) => {
      const split = new SplitText(el, { type: "chars" });
      el.addEventListener("mouseenter", () => {
        gsapWithCSS.set(split.chars, { filter: "blur(8px)" });
        gsapWithCSS.to(split.chars, {
          filter: "blur(0px)",
          duration: 0.15,
          ease: "power2.out",
          stagger: 0.025,
  ...
  // src/utils/animations/gsap/serviceStack.ts
  var PEEK_Y = [0, 140, 240];
  var EXIT_ROTATE_X = 50;
  var STACK_SCALE = 0.85;
  var PERSPECTIVE = 2500;
  function initServiceStack() {
    const cards = gsapWithCSS.utils.toArray(".home_services_cards");
    if (!cards.length) return;
    const section = document.querySelector(".section_home_services");
    if (!section) return;
  ...
  // src/utils/animations/gsap/typeEffect.ts
  ...
  // src/utils/animations/swiper/swiperFactory.ts
  var BASE_OPTIONS = {
    speed: 750,
    loop: true,
    grabCursor: true,
  ...
  function initSwipers() {
    const variants = Object.keys(VARIANT_OPTIONS);
    variants.forEach((variant) => {
      const swipers = document.querySelectorAll(`.swiper.is-${variant}`);
      if (!swipers.length) return;
      const options = { ...BASE_OPTIONS, ...VARIANT_OPTIONS[variant] };
  ...
  // src/utils/animations/swiper/swiperPortfolio.ts
  function initSwiperPortfolio() {
    const els = document.querySelectorAll(".swiper.is-portfolio");
  ...
  // src/utils/animations/swiper/swiperSectors.ts
  function initSwiperLoop() {
    const marquees = document.querySelectorAll(".swiper.is-loop");
  ...
  // src/utils/global/modalviewers.ts
  ...
  function initResetPosition() {
    const modelViewers = document.querySelectorAll(".glb_coin");
  ...
  var SHRINK = 0.35;
  var EXPAND = 0.45;
  var TRIGGER_DELAY = 350;
  var EASE_IN_OUT = "power2.inOut";
  ...
  function initNavbarShrink() {
    ...
    const feedLink = document.querySelector(".nav_menu_link.is-feed");
    const ctaLink = document.querySelector(".nav_menu_link.is-cta");
    ...
    if (ctaLink) gsapWithCSS.set(ctaLink, { display: "none", opacity: 0 });
    ...
    function triggerShrink() {
      ...
      activeTl = gsapWithCSS.timeline().to(menu, { x: -20, opacity: 0, duration: FADE_OUT, ease: EASE_IN_OUT }, 0).to(fullElements, { opacity: 0, duration: FADE_OUT, ease: EASE_IN_OUT }, 0).call(() => {
        menu.style.display = "none";
        if (brand) gsapWithCSS.set(brand, { display: "none" });
        if (feedLink) gsapWithCSS.set(feedLink, { display: "none" });
        if (ctaLink) gsapWithCSS.set(ctaLink, { display: "flex", opacity: 0 });
        if (navCta) gsapWithCSS.set(navCta, { margin: "0 auto" });
        gsapWithCSS.set(container, { display: "flex" });
      }).to(container, { width: compactWidth, duration: SHRINK, ease: EASE_IN_OUT }, ">").to(compactElements, { opacity: 1, duration: FADE_IN_CTA, ease: EASE_OUT }, "<");
    }
    function triggerExpand() {
      isShrunk = false;
      if (activeTl) activeTl.kill();
      activeTl = gsapWithCSS.timeline().call(() => {
        gsapWithCSS.set(container, { overflow: "hidden" });
        menu.style.display = "";
        gsapWithCSS.set(menu, { x: -20, opacity: 0 });
        if (brand) gsapWithCSS.set(brand, { display: "", opacity: 0 });
        if (feedLink) gsapWithCSS.set(feedLink, { display: "", opacity: 0 });
        if (navCta) gsapWithCSS.set(navCta, { margin: "" });
      }).to(compactElements, { opacity: 0, duration: FADE_OUT, ease: EASE_IN_OUT }, "<").to(container, { width: initialWidth, duration: EXPAND, ease: EASE_IN_OUT }, "<").call(() => {
        if (ctaLink) gsapWithCSS.set(ctaLink, { display: "none" });
        gsapWithCSS.set(container, { display: "", width: "", overflow: "" });
        if (navCta) gsapWithCSS.set(navCta, { opacity: 0 });
      }).to(menu, { x: 0, opacity: 1, duration: FADE_IN, ease: EASE_OUT }, ">").to(fullElements, { opacity: 1, duration: FADE_IN, ease: EASE_OUT }, "<");
    }
    ScrollTrigger2.create({
      start: "top top",
      end: "max",
  ...
  function initPopupContact() {
    ...
    function openPopup() {
      ...
      gsapWithCSS.set([popupComponent, popupWrapper, popupBackground], { opacity: 0 });
      gsapWithCSS.set(popupWrapper, { y: "2rem" });
      gsapWithCSS.to(popupComponent, { opacity: 1, duration: 0.3, ease: "power2.out" });
      gsapWithCSS.to(popupBackground, { opacity: 1, duration: 0.6, ease: "power2.out" });
      gsapWithCSS.to(popupWrapper, { opacity: 1, y: 0, duration: 0.6, ease: "power2.out" });
    }
    function closePopup() {
      gsapWithCSS.to(popupWrapper, { y: "2rem", opacity: 0, duration: 0.3, ease: "power2.in" });
      gsapWithCSS.to(popupComponent, {
        opacity: 0,
        duration: 0.3,
        ease: "power2.in",
        onComplete: () => {
  ...
  // src/utils/global/threeSection.ts
  var IFRAME_SRC = "https://lezar-studio.github.io/GhostStudio_Carousel/";
  var INJECTED_ATTR = "data-carousel-injected";
  function observeAndInject(container) {
